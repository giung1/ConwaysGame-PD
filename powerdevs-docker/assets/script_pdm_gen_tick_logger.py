...
def get_neighbor_count(r, c, rows, cols):
    """Cuenta vecinos validos (bordes muertos) para el parametro neighbors."""
    count = 0
    for dr in [-1, 0, 1]:
        for dc in [-1, 0, 1]:
            if dr == 0 and dc == 0:
...
def generar_pdm(rows, cols, alive_set, rules):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            n_count = get_neighbor_count(r, c, rows, cols)
            ...
            lines.append(f"                neighbors = Str; {n_count} ; ")
            lines.append("                }")
            lines.append("            }")
...
class App(tk.Tk):
    ...
    def toggle(self, r, c):
        ...
        if alive:
            ...
            # Si muere -> Verde Oscuro (¡IMPORTANTE!)
